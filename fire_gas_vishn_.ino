...
void sendPhotoTelegram(String message);
...
void loop(){
  ...
  int flameDetected = digitalRead(FIRE_SENSOR_PIN);
  if (flameDetected == LOW) {
    Serial.println("Flame detected!");
    sendPhotoTelegram("Flame detected, be careful!");
    sendPhoto = true;
  }
  ...
  if (gasLevel >= 28) {
    ...
    sendPhoto = true;
    sendPhotoTelegram("High gas level detected, be careful!");
  }
  if (sendPhoto){
